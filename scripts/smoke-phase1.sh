...
JOB_ID=$(echo "$JOBS_RESPONSE" | node -e "const d=JSON.parse(require('fs').readFileSync(0,'utf8')); const first=d.data?.jobs?.[0]?.id; if(first) console.log(first);")

if [[ -n "${JOB_ID}" ]]; then
	curl -sf -H "${AUTH_HEADER}" "${API_URL}/api/jobs/${JOB_ID}" >/dev/null || fail "GET /api/jobs/:id failed"
else
	echo "[smoke] no jobs yet; skipping job detail check"
fi
